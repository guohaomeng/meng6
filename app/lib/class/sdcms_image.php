...
final class sdcms_image
{
    ...
    #生成缩略图
    public function thumb($old,$width=200,$height=200,$new)
    {
        if(!self::check())
        {
            return WEB_ROOT.$old;
        ...
        $x=$width/$info[0];
        $y=$height/$info[1];
        $is_small=($info[0]<=$width&&$info[1]<=$height);
        if($is_small)
        {
          $w=$info[0];
          $h=$info[1];
        ...
        else
        {
            #按宽度等比缩放
            $w=$width;
            $h=(ceil($info[0]/$width*$info[1])>$height)?$height:(ceil($info[1]/$height*$info[1]));
        }
