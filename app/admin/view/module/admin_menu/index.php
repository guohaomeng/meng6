...
<body>
    <div class="position">当前位置：系统管理 > <a href="{THIS_LOCAL}">菜单管理</a></div>
    <div class="border">
        <!---->
        <div class="am-btn-group">
            <a href="{U('add')}" class="am-btn am-btn-secondary am-radius am-text-sm"><span class="am-icon-plus am-margin-right-sm"></span>添加菜单</a>
        </div>
        ...
        <table class="am-table am-table-hover am-margin-top">
            ...
            <tbody>
            {sdcms:rp top="0" table="sd_admin_menu" where="followid=0" order="ordnum,id"}
            {php $classid=$rp[id]}
            <tr>
                ...
                <td>{$rp[id]}</td>
                <td>{$rp[title]}</td>
                <td>{iif($rp[islock]==1,'显示','<em>隐藏</em>')}</td>
            ...
            {sdcms:rs top="0" table="sd_admin_menu" where="followid=$classid" order="ordnum,id"}
            <tr>
                ...
                <td>{$rs[id]}</td>
                <td>　　{$rs[title]}</td>
                <td>{iif($rs[islock]==1,'显示','<em>隐藏</em>')}</td>
