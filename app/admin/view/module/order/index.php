...
<body>
    <div class="position">当前位置：扩展管理 > <a href="{THIS_LOCAL}">订单管理</a></div>
    <div class="border">
        <!---->
        ...
        <div class="am-btn-group am-btn-group-sm am-btn-toolbar am-margin-left-xs">
            ...
            <a class="am-btn am-btn-{if $type==1}warning{else}default{/if} am-radius" href="{U('index','type=1')}">未付款</a>
            <a class="am-btn am-btn-{if $type==2}warning{else}default{/if} am-radius" href="{U('index','type=2')}">已付款</a>
            <a class="am-btn am-btn-{if $type==3}warning{else}default{/if} am-radius" href="{U('index','type=3')}">未处理</a>
            <a class="am-btn am-btn-{if $type==4}warning{else}default{/if} am-radius" href="{U('index','type=4')}">已处理</a>
        </div>
        <form action="{THIS_LOCAL}" class="am-form am-form-inline am-fr">
        {if sdcms[url_mode]==1}<input type="hidden" name="m" value="{C('ADMIN')}" /><input type="hidden" name="c" value="order" /><input type="hidden" name="a" value="index" />{/if}
        ...
        <table class="am-table am-table-hover am-margin-top">
            ...
            <tr>
                ...
                <td>{iif($rs[ispay]==1,'已付款','<em>未付款</em>')} {iif($rs[isover]==1,'已处理','<em>未处理</em>')}</td>
