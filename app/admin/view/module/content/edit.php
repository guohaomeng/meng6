            ...
            <div class="am-tabs-bd am-tabs-bd-ofv">
                ...
                <div class="am-tab-panel{if $index==0} am-active{/if}">
                    ...
                    <div class="am-form-group">
                        ...
                        <div class="am-u-sm-10">
                            <label class="am-radio-inline"><input type="radio" name="isauto" id="isauto_0" value="0"{if $record['isauto']==0} checked{/if}><span for="isauto_0">否</span></label>
                            <label class="am-radio-inline"><input type="radio" name="isauto" id="isauto_1" value="1"{if $record['isauto']==1} checked{/if}><span for="isauto_1">是</span></label>                                                        							
                    ...
                    <div class="am-form-group"{if $rs['field_type']==7} style="display:none;"{/if}>
                        ...
                        <div class="am-u-sm-10">
                            ...
                            {case 1}<input type="text" name="{$rs['field_key']}" id="{$rs['field_key']}" size="50"{if $rs['field_length']!=0} maxlength="{$rs['field_length']}"{/if} value="{$record[$rs['field_key']]}" {deal_rule($rs['field_rule'],$rs['field_title'])}>{if $rs['field_key']=='url'}　<span class="am-btn-group"><button type="button" class="am-btn am-btn-secondary dropzone" config="{$rs['field_key']}" url="{U('upload/upfile','type=3')}" maxsize="{C('upload_file_max')}" title="上传">上传</button><button type="button" class="am-btn am-btn-primary am-radius fm-choose" data-name="{$rs['field_key']}" data-url="{U('upload/imagelist','type=3&multiple=0')}" data-type="3" data-multiple="0" title="选择">选择</button></span>{/if}{if $rs['field_key']=='tags'}　<span class="am-btn-group"><button type="button" class="am-btn am-btn-primary am-radius fm-tags" data-name="{$rs['field_key']}" data-url="{U('taglist')}" title="选择">选择</button></span>{/if}{/case}
                            {case 2}<input type="text" name="{$rs['field_key']}" id="{$rs['field_key']}" size="50"{if $rs['field_length']!=0} maxlength="{$rs['field_length']}"{/if} value="{date('Y-m-d H:i:s',$record[$rs['field_key']])}" class="my-datetimepicker-se" {deal_rule($rs['field_rule'],$rs['field_title'])}>{/case}
                            ...
                            {case 7}<input type="text" name="{$rs['field_key']}" id="{$rs['field_key']}" size="50"{if $rs['field_length']!=0} maxlength="{$rs['field_length']}"{/if} value="{$record[$rs['field_key']]}">{/case}
                    ...
                    <div class="am-form-group">
                        ...
                        <div class="am-u-sm-10">
                            ...
                            <label class="am-checkbox-inline">
                                <input type="checkbox" name="view_lever[]" id="view_lever_{$rs[gid]}" value="{$rs[gid]}"{if in_array($rs[gid],explode(',',$record['view_groupid']))} checked{/if}><span for="view_lever_{$rs[gid]}">{$rs[gname]}</span>
                            </label>
                            {/sdcms:rs}
                        </div>
                    </div>
                ...
                <div class="am-tab-panel">
					...
					<div class="am-form-group">
                        ...
                        <div class="am-u-sm-10">
							{switch $rs['field_type']}
                            {case 1}<input type="text" name="extend[{$rs['field_key']}]" id="{$rs['field_key']}" size="50" value="{if isset($extend[$rs['field_key']])}{$extend[$rs['field_key']]}{/if}" >{/case}
                            {case 2}
