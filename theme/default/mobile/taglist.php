    ...
    <article>
    	<section>
            ...
            <ul class="home_news">
                ...
                <li><span class="date">{date('m-d',$rs[createdate])}</span><div class="right"><a href="{$rs[link]}" title="{$rs[title]}" class="text-hide">{str_replace($tagname,"<font color=red>$tagname</font>",$rs[title])}</a><p class="text-hide">{cutstr(nohtml($rs[intro]),200)}</p></div></li>
                {/sdcms:rs}
             </ul>
             <div class="clear"></div>
