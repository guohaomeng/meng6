...
<body>
    ...
    <div class="width inner_container am-animation-slide-bottom am-animation-delay-1">
        ...
        <ul class="news_list mt20">
           ...
           <li><span class="date">{date('d',$rs[createdate])}<em>{date('Y',$rs[createdate])}-{date('m',$rs[createdate])}</em></span><div><a href="{$rs[link]}" title="{$rs[title]}" target="_blank">{$rs[title]}</a>{cutstr(nohtml($rs[intro]),500,1)}</div></li>
           {/sdcms:rs}
         </ul>
         <div class="clear"></div>
