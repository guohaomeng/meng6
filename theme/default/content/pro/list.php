    ...
    <div class="width inner_container am-animation-slide-bottom am-animation-delay-1">
        ...
        <div class="home_pro">
            <ul>
                ...
                {rs:eof}暂时没有产品{/rs:eof}
                <li><a href="{$rs[link]}" title="{$rs[title]}" target="_blank"><div><img src="{thumb($rs[pic],280,280)}" alt="{$rs[title]}" height="279"></div><p class="title">{cutstr($rs[title],90,1)}</p><p class="price"><span>人气：{$rs[hits]}</span>{if $rs[price]!=0}¥ {$rs[price]}{else}面议{/if}</p></a></li>
                {/sdcms:rs}
            </ul>
            <div class="clear"></div>
        </div>
